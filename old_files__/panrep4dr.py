...
def finetune_pr_for_link_prediction(train_g, test_g, train_edges, valid_edges, test_edges, model, batch_size,
    ...
    folder = 'saved_model/'
    model_name = "fin_model" + str(datetime.date(datetime.now())) + "-" + str(
        datetime.time(datetime.now()))
    th.save(finetune_lp_model.state_dict(), folder + model_name)
    return
...
def _fit(args):
    ...
    lr_lp_ft=lr
    finetune_pr_for_link_prediction(train_g, test_g, train_edges, valid_edges, test_edges, model, batch_size,
                                    n_hidden, ntype2id, ng_rate, fanout, l2norm,
                                    n_layers, n_lp_fintune_epochs, lr_lp_ft, use_cuda, device,
                                    learn_rel_embed=learn_rel_embed)
    eval_lp=False
    if eval_lp:

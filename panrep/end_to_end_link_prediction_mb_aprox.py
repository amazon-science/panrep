...
def _fit(n_epochs, n_layers, n_hidden, n_bases, fanout, lr, dropout,
    ...
    for epoch in range(n_epochs):
        ...
        optimizer.zero_grad()
        for i, (seeds, blocks) in enumerate(loader):
            embeddings = model.forward_mb(blocks)
            loss=model.link_predictor.forward_mb(blocks[-1],embeddings)
            loss.backward()
            optimizer.step()
                                                                                ...
                                                                                for k_fold in K_list:
                                                                                    for ng_rate in ng_rate_list:
                                                                                        ...
                                                                                        results[(n_epochs,
                                                                                                 ...
                                                                                                 use_self_loop,
                                                                                                 k_fold, ng_rate,test_edge_split)]=acc


        results[str(args)]=1
        file=args.dataset+'-'+str(datetime.date(datetime.now()))+"-"+str(datetime.time(datetime.now()))
        pickle.dump(results, open(os.path.join("results/end_to_end_link_prediction/", file + ".pickle"), "wb"),
                    protocol=4)
        return
